...
// functions -------------------------------------------------------------------
exports.isHttpInNode = function (flowId, node) {
  if (node.z === flowId &&
      node.type === 'http in') {
    return true
  }
...
exports.createComment = function (flowId, title, detail, x, y) {
...
exports.createHttpNode = function (flowId, swaggerDocNodeId, url, method, outputLabel, x, y) {
  // create http nodes
...
exports.createResponseNode = function (flowId, x, y) {
...
exports.convertUrl = function (basePath, path) {
  return (convertString(basePath) + path).replace(/\{[^}]*\}/g, '*')
}
